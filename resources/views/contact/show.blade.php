...
@section('content')
                    ...
                    <div class="tab-content">
                        ...
                        <div class="tab-pane
                                ...
                                @else
                                    '' @endif"
                            id="loans_tab">
                            @include('contact.partials.loans_tab')
                        </div>
                        @if (in_array($contact->type, ['both', 'supplier']))
                            <div class="tab-pane
                            @if (!empty($view_type) && $view_type == 'purchase') active
    ...
    @include('contact.partials.add_payment')
    @include('contact.partials.index_credits')
    @include('contact.partials.payment_credits')
    @include('contact.partials.create_credits')
    @include('contact.partials.edit_credits')
...
@section('javascript')
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            $(document).on('submit', 'form#add_payment_modal, form#add_discount_modal, form#edit_discount_form',
                function(e) {
                    console.log("aaaaa");
                    e.preventDefault();
                    var form = $(this);
                    ...
                    $.ajax({
                        ...
                        success: function(result) {
                            if (result.success === true) {
                                console.log(result);
                                $('div#add_payment_modal').modal('hide');
            ...
            $(".credit_index_btn").click(function() {
                $.ajax({
                    method: 'GET',
                    url: $(this).data('url'),
                    dataType: 'json',
                    success: function(data) {
                        if (data.status == true) {
                            ...
                            let loanList = $("#loan_list");
                            let loan = $("#pre-loan").clone();
                            $(".loan").remove();
                            data.loans.forEach(function(loanData) {
                                //settear datos
                                loan.removeAttr("id").removeClass("hide").addClass("loan");
                                loan.find(".id_loan").text(loanData.id);
                                loan.find(".amount_loan").text(loanData.amount);
                                loan.find(".users_loan").text(loanData.first_name + " " + loanData.last_name);
                                loan.find(".date_loan").text(loanData.created_at);
                                loanList.append(loan
                            .clone()); // Clonar y agregar para cada iteración
                            });
                        } else {
            ...
            $(".credit_payments_index_btn").click(function() {
                $.ajax({
                    method: 'GET',
                    url: $(this).data('url'),
                    dataType: 'json',
                    success: function(data) {
                        if (data.status == true) {
                            let loanList = $("#loanListPayment");
                            let loan = $("#preLoanPayment").clone();
                            $(".loan").remove();
                            data.loans.forEach(function(loanData) {
                                loan.removeAttr("id").removeClass("hide").addClass("loan");
                                loan.find(".id_loan").text(loanData.id);
            ...
            $("#credit_payment_btn").click(function (){
                $.ajax({
                    method: 'GET',
                    url: $(this).data('url'),
                    dataType: 'json',
                    success: function(data) {
                        $(".sum_credits").val(data.sum);
                    },
                    error: function(error) {
                        console.error("Error en la llamada AJAX:", error);
                    }
        ...
        //FUNCION DONDE SE SACA LOS PAGOS
